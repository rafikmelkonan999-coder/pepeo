...
def get_kb(key):
    key = normalize(key)
    stats = STATS_DB.get(key, {"likes": 0, "dislikes": 0})
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        ...
        f"📊 Всего записей в базе: {total_users}\n\n"
        f"Введите юзернейм или ID для поиска:",
...
async def ask_value(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("Введите число (количество лайков/дизов):")
...
async def process_admin_action(message: types.Message, state: FSMContext):
    ...
    if target not in STATS_DB: STATS_DB[target] = {"likes": 0, "dislikes": 0}
...
async def handle_feedback(callback: types.CallbackQuery):
    ...
    key = normalize(key)
    if key not in STATS_DB: STATS_DB[key] = {"likes": 0, "dislikes": 0}
...
async def search(message: types.Message):
    ...
    try:
        ...
        # Визуальное оформление как на скриншоте
        if msg_found:
            text = (
                f"<b>SCAM</b> @{query}\n"
                f"Искали: 🟦 {query}\n"
                ...
                f"Добавлен в скам-базу <a href='https://t.me/{TARGET_CHANNEL}/{msg_found.id}'>@GID_ScamBase</a> (пруфы по ID). "
                f"Сделки недопустимы."
            )
        else:
            text = f"👤 @{query}\nСтатус: <b>Репутация: чист.</b>"
